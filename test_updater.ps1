...
function Test-PackageLayout {
    ...
    $sourceVersion = (Get-Content -LiteralPath (Join-Path $PSScriptRoot 'version.json') -Raw | ConvertFrom-Json).version
    Assert-Equal $sourceVersion ((Get-Content -LiteralPath (Join-Path $distRoot 'app\version.json') -Raw | ConvertFrom-Json).version) 'wrong packaged version'
